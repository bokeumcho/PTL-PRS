...
vector<vector<float>> BedFileReader::calculatePRS_mat(vector<string> snpList, vector<vector<float>> betaAll, vector<int> sampleList = vector<int>()){
    ...
    vector<int> oneSnp; 
    int models = betaAll[1].size();

    vector<vector<float>> PRS(models, vector<float> (m_line_counter, 0));
     // models * samples
    ...
    for (int k = 0; k < models; k++){
        for (int i = 0; i < snpList.size(); i++){
            ...
            if (!sampleList.empty()) {
                ...
                for (const int& sampleIdx : sampleList){
                    if (sampleIdx!=-1){
                        PRS[j][k] += oneSnp[sampleIdx] * betaAll[k][i];
                    }
                    // PRS of NA remains 0
                    j++;
                }
...
RcppExport SEXP BedFileReader__readSomeSnp(SEXP xp, SEXP snpNames_, SEXP sampleList_){
    Rcpp::XPtr<BedFileReader> reader(xp);

    vector<string> snpNames = Rcpp::as<vector<string>>(snpNames_);
    vector<int> sampleList = Rcpp::as<vector<int>>(sampleList_);

    vector<vector<int>> genomat = reader->readSomeSnp(snpNames, sampleList);
    return Rcpp::wrap(genomat);
}
