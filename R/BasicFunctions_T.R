...
Calculate_PRS_direct <- function(ped_val, beta.info, beta.all){
    ...
    train_file = paste0('/media/leelabsg-storage0/DATA/UKBB/imp/Other_format/ukb_imp_chr',beta.info$CHR[1],'_v3')
    
    # declare global variable
    BedFileReader <<- new( "BedFileReader", paste0(train_file,".fam"), paste0(train_file,".bim"), paste0(train_file,".bed"))

    result = try(BedFileReader$snp_index_func(),silent=T)
   
    #source("/media/leelabsg-storage1/bokeum/TLPRS/complication/BasicFunctions.R")

    cat('calculate PRS for CHR',beta.info$CHR[1],'\n')

    #    user  system elapsed 
    #   401.554   7.431 625.744 
    system.time({ G_val = readSomeSnp(beta.info$SNP, ped_val$ukb_idx) })

    # handling NA (mean imputation)
    G_val1 <- apply(G_val, 1, function(x){
      #x[is.na(x)] <- mean(x, na.rm=TRUE)
      x[which(x==9)] <- mean(x[which(x!=9)])
      return(x)
    })

    cat(dim(G_val1), dim(beta.all),'\n')

    PRS.all = as.matrix(G_val1) %*% as.matrix(beta.all)
    
    return(PRS.all)
}

Calculate_PRS_direct_ldb <- function(ped_val, beta.info, beta.all, num){
    sourceCpp("/media/leelabsg-storage1/bokeum/TLPRS/source/BedFileReader.cpp")
    setClass("BedFileReader", representation( pointer = "externalptr" ) )
    ...
    # cat('calculate PRS for CHR',beta.info$CHR[1],'\n')
    cat ('calculate PRS for LDblock',num, '\n')
...
PRStr_tuning<-function(Beta.all, ped_val_file, Covar_name,Y_name, Ytype, lr_list, iter){
    ...
    # ### by LDblocks
    # total_PRS <- matrix(0, nrow=dim(ped)[1], ncol=dim(beta.all)[2])  # Adjust this initialization based on the actual structure of your result

    # # Loop through each unique value in LDblocks2[[1]]
    # for (i in unique(LDblocks2[[1]])) {
    #     # Calculate the partial result
    #     partial_PRS <- Calculate_PRS_direct_ldb(
    #         ped, 
    #         beta.info[which(LDblocks2[[1]] == i), ], 
    #         beta.all[which(LDblocks2[[1]] == i), ], 
    #         which(i == unique(LDblocks2[[1]]))
    #     )
    #     print(dim(partial_PRS))
    #     # Add the partial result to the cumulative total
    #     total_PRS <- total_PRS + partial_PRS  # Adjust this operation based on how results are combined
    #     gc()
    # }
    ...
    PRS.all = cbind(ped[,c('FID','IID')],PRS.all)
    ...
    write.table(out.item[,1:3],file=paste0(tempfile,"_Rauc_noCovar_val.txt"),row.names=F,quote=F,col.names=T)
    #out.item.val=fread(paste0(tempfile,"_Rauc_noCovar_val.txt"))
